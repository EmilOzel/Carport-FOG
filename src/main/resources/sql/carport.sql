...
CREATE TABLE orders (
    ...
    status      VARCHAR(20)   NOT NULL DEFAULT 'pending'
                              CHECK (status IN ('pending', 'offer', 'approved', 'rejected', 'completed')),
...
CREATE TABLE carport (
    ...
    width      SMALLINT NOT NULL CHECK (width  BETWEEN 240 AND 780),
    length     SMALLINT NOT NULL CHECK (length BETWEEN 240 AND 1200)
);

CREATE TABLE shed (
    shed_id    SERIAL   PRIMARY KEY,
    carport_id INT      NOT NULL UNIQUE REFERENCES carport(carport_id),
    width      SMALLINT NOT NULL CHECK (width  BETWEEN 180 AND 720),
    length     SMALLINT NOT NULL CHECK (length BETWEEN 150 AND 690)
);
...
CREATE TABLE material_line (
    material_line_id SERIAL        PRIMARY KEY,
    carport_id       INT           NOT NULL REFERENCES carport(carport_id),
